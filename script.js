...
const first_profile_name = document.querySelector(".first-user");
const second_profile_name = document.querySelector(".second-user");
const compare_section = document.querySelector(".compare-main");
let stars_1 = 0,
  stars_2 = 0,
  fork_1 = 0,
  fork_2 = 0;
...
function calculate_and_compare_stats(first, second) {
  ...
  // for language sets
  first.forEach(function (repo) {
    if (repo.language) {
      if (repo.language in language_data_1) {
        language_data_1[repo.language] += 1;
      } else {
        language_data_1[repo.language] = 1;
      }
    }
  ...
  second.forEach(function (repo) {
    if (repo.language) {
      if (repo.language in language_data_2) {
        language_data_2[repo.language] += 1;
      } else {
        language_data_2[repo.language] = 1;
      }
    }
  ...
  set_of_languages = [
    ...new Set([
      ...Object.keys(language_data_1),
      ...Object.keys(language_data_2),
    ]),
  ];
}
...
function compare_id(first_person, second_person) {
  ...
  <!-- first section which will contain the image and other details  -->
     <div class="pictures-profile-details flex gap-8 mt-5 shadow-xl items-stretch flex-wrap md:flex-nowrap ">

  <!-- The first ID details -->
  <div class="first-user-data md:w-[50%] flex flex-col justify-between w-[100%]">
    <div
    ...
    >
      ...
      <div class="name-email self-center">
        ...
        <div class="web-link text-white">
          ${company_of_user(first_person)}
        </div>
      </div>
    </div>
    <div class="about-section bg-white py-5 px-5 text-gray-700 flex-1 rounded-b-xl">
      <div class="about">${bio_of_user(first_person)}</div>
      <div class="followers-repos flex mt-5 px-4 gap-5">
        ...
        >
          <div class="icon-numbers flex items-center space-x-2">
            ...
            <span class="text-blue-700 text-lg font-semibold">
              ${first_person.followers}
            </span>
          </div>
        ...
        <div
          class="repos-section bg-[#F9FAFB] w-full flex flex-col items-center justify-center py-4 rounded-lg h-full"
        >
          <div class="icon-numbers flex items-center space-x-2">
            <span class="material-symbols-outlined text-blue-700 text-xl">
              inventory_2
      ...
      <div class="other-details flex flex-col gap-4 mt-5">
        ...
        <div class="flex gap-2">
          <span class="material-symbols-outlined text-blue-700"> link </span>
          <div class="portfolio-1">
            <a href="${first_person.html_url}">${first_person.login}</a>
          </div>
        </div>
        <div class="flex gap-2">
          <span class="material-symbols-outlined text-blue-700">
          ...
          <div class="join-date-1">
            ${formatDate(first_person.created_at)}
          </div>
        </div>
      </div>
    </div>
  ...
  <!-- Second ID data -->
  <div class="second-user-data md:w-[50%] flex flex-col justify-between w-[100%]">
    <div
      class="blue-part bg-blue-600 py-5 pl-4 rounded-t-xl flex gap-3"
    >
      <div class="image2">
        <img
      ...
      <div class="name-email self-center">
        ...
        <div class="web-link text-white">
          ${company_of_user(second_person)}
        </div>
      </div>
    </div>
    <div class="about-section bg-white py-5 px-5 text-gray-700 flex-1 rounded-b-xl">
      <div class="about">${bio_of_user(second_person)}</div>
      <div class="followers-repos flex mt-5 px-4 gap-5">
        <!-- Followers Section -->
        <div
          class="followers-section bg-[#F9FAFB] w-full flex flex-col items-center justify-center py-4 rounded-lg"
      ...
      <div class="other-details flex flex-col gap-4 mt-5">
        ...
        <div class="flex gap-2">
          <span class="material-symbols-outlined text-blue-700"> link </span>
          <div class="portfolio-1">
            <a href="${second_person.html_url}">${second_person.login}</a>
          </div>
        </div>
        <div class="flex gap-2">
          <span class="material-symbols-outlined text-blue-700">
          ...
          <div class="join-date-1">
            ${formatDate(second_person.created_at)}
          </div>
        </div>
      </div>
    </div>
  ...
  let table_data = document.querySelector(".language-data");
  for (let i = 0; i < set_of_languages.length; i++) {
    table_data.innerHTML += `
    <tr>
      <td class="cpp px-4 py-2 text-gray-600 border-b">${
        set_of_languages[i]
      }</td>
      <td class="px-4 py-2 text-center border-b  "><span class= "bg-purple-300 text-purple-950 rounded-full p-2">${
        language_data_1[set_of_languages[i]] || 0
      }</span></td>
      <td class="px-4 py-2 text-center border-b"><span class="bg-blue-300 text-blue-950 rounded-full p-2">${
        language_data_2[set_of_languages[i]] || 0
      }</span></td>
    </tr>`;
  }
}
...
async function fetching_data() {
  const firstProfile = first_profile_name.value.trim();
  const secondProfile = second_profile_name.value.trim();
  const compareMain = document.querySelector(".compare-main");
  ...
  if (!firstProfile || !secondProfile) {
    compareMain.innerHTML = `
      <div class="error-message text-center p-4 bg-red-200 text-red-700 rounded-lg text-xl">
        Please enter both GitHub usernames to compare.
      </div>`;
    return;
  }
  ...
  compareMain.innerHTML = `
    <div class="loading text-center p-4 bg-blue-200 text-blue-700 rounded-lg">
      Fetching data... Please wait.
    </div>`;

  try {
    
    let fetch_data1 = await fetch(`https://api.github.com/users/${firstProfile}`);
    if (!fetch_data1.ok) throw new Error(`User ${firstProfile} not found`);
    fetch_data1 = await fetch_data1.json();

    let fetch_data2 = await fetch(`https://api.github.com/users/${secondProfile}`);
    if (!fetch_data2.ok) throw new Error(`User ${secondProfile} not found`);
    ...
    if (error.message.includes("not found")) {
      compareMain.innerHTML = `
        <div class="error-message text-center p-4 bg-yellow-200 text-yellow-700 rounded-lg">
          ${error.message}.
        </div>`;
    } else {
      compareMain.innerHTML = `
        <div class="error-message text-center p-4 bg-red-200 text-red-700 rounded-lg">
